...
- **feat(hook): proactive DIY detection** — PreToolUse `Write|Edit|MultiEdit` hook scores hand-rolled capability code, gates on confidence/recurrence, validates via `runAdvise`, injects candidates + facts + migration guidance; denies only on org `diy_category` policy. Wired for Claude Code (global), Cursor (`.cursor/hooks.json`), and VS Code Copilot (`.github/hooks/starlog.json`).
